...
export const TELEMETRY_FIELD_CONFIG: Record<string, { label: string; fields: Record<string, string> }> = {
  gps: {
    label: "GPS",
    fields: {
      rx_time: "RX Time",
  ...
  battery: {
    ...
    fields: {
      ...
      high_cell_t: "High Cell T",
      cell_idx_low_v: "IDX Low Voltage",
      cell_idx_high_t: "IDX High T"
    }
  },
  mppt1: {
    label: "MPPT 1", 
...
export const CUSTOM_FIELD_CONFIG: SelectGroup = {
  label: "Custom",
  options: [
    { value: "mph", label: "Miles per Hour", dataPath: "gps.speed" },
    { value: "kph", label: "Kilometers per Hour", dataPath: "gps.speed" },
    { value: "soc", label: "State of Charge", dataPath: "custom.soc" },
    { value: "mpc", label: "Motor Power Consumption", dataPath: "custom.motorPower" }
  ]
};
...
export function generateSelectGroups(): SelectGroup[] {
  ...
  Object.entries(TELEMETRY_FIELD_CONFIG).forEach(([category, config]) => {
    const options: SelectOption[] = Object.entries(config.fields).map(([field, label]) => ({
...
export function getValueFromPath(data: TelemetryData, path: string, value?: string): number | undefined {

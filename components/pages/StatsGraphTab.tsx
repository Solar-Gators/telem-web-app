...
const GAP_BREAK_MS = 12 * 60 * 60 * 1000;

function isValidNumber(value: unknown): value is number {
  return typeof value === "number" && Number.isFinite(value);
}

function isIsolatedPoint(data: any[], index: number) {
  const curr = data[index];
  if (!curr || curr.__gap || !isValidNumber(curr.ts)) return false;
  ...
  const prevClose =
    prev &&
    !prev.__gap &&
    isValidNumber(prev.ts) &&
    Math.abs(curr.ts - prev.ts) <= GAP_BREAK_MS;
  const nextClose =
    next &&
    !next.__gap &&
    isValidNumber(next.ts) &&
    Math.abs(next.ts - curr.ts) <= GAP_BREAK_MS;

  return !prevClose && !nextClose;
}

function insertGapBreaks(data: any[], keys: string[]) {
  if (!data || data.length === 0) return data;

  const normalized = data
    .map((point) => {
      const ts = isValidNumber(point?.ts)
        ? point.ts
  ...
  const output: any[] = [sorted[0]];

  for (let i = 1; i < sorted.length; i++) {
    const prev = sorted[i - 1];
    const curr = sorted[i];

    if (curr.ts - prev.ts > GAP_BREAK_MS) {
      const gapTs = prev.ts + 1;
      const gapPoint: any = {
        ts: gapTs,
        timestamp: new Date(gapTs),
        __gap: true,
      };
      keys.forEach((key) => {
        gapPoint[key] = null;
      });
      output.push(gapPoint);
    }
    ...
    if (selectedDataKeys.length > 0 && startDate && endDate) {
      ...
      if (customFields.length > 0) {
        ...
        fetchTelemetryDataInRange(startDate, endDate).then((allData) => {
          ...
          setChartData(insertGapBreaks(processedData, selectedDataKeys));
        });
      } else {
        ...
        Promise.all(
          ...
          results.forEach(({ key, data }) => {
            data?.forEach((point: any) => {
              ...
              const iso = cdtTimestamp.toISOString();
              ...
              mergedData[iso][key] = point.value;
            });
          });
          ...
          setChartData(insertGapBreaks(mergedArray, selectedDataKeys));
        });
      }
    }
            ...
            return (
              <Line
                ...
                dot={(props) => {
                  ...
                  if (!point || point.__gap) return null;
                  if (point[key] == null) return null;
                  if (!isIsolatedPoint(filteredChartData, index)) return null;
                  return (
                    <circle
                      cx={props.cx}
                      cy={props.cy}
